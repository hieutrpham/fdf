...
t_point2d	*convert_3d_to_2d(int *map, t_map *map_data)
{
	...
	unsigned int	y;
	unsigned int	x;
	float scale = WIDTH/get_diagnal(map_data) * cos(30.0f / 180.0f * M_PI);
...
t_point2d	project_isometric(t_point3d p3d, t_map *map, float scale)
{
	...
	float cos30 = cos(30.0f / 180.0f * M_PI);
	float sin30 = sin(30.0f / 180.0f * M_PI);
	p3d.x -= map->width * 0.5f;
	p3d.y -= map->height * 0.5f;
	p2d.x = (int) ((cos30 * p3d.x - cos30 * p3d.y) * scale + (float)WIDTH / 2.f);
	p2d.y = (int) ((sin30 * p3d.x + sin30 * p3d.y - p3d.z) * scale + (float)HEIGHT / 2.f);
	return (p2d);
}
